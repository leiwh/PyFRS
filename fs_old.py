...
#-------Initialize the time, gamma, Fv arries in Log space
    ...
    num_obs=len(time_obs)
    index_obs=np.zeros(1,100,num_obs)
    for i in range(0,num_obs):
        ...
        listt.sort()
        t=listt
        indexo=listt.index(t_obs)
        index_obs[i]=indexo

    inum_max=index_obs[num_obs]
    inum=len(t)
...
#        t=np.logspace(-4., 3.0,inum)
        ...
        lgFv=np.logspace(1.,6.,num_obs)*np.ones((len(vlist),num_obs))
        lgFv_SSC=np.logspace(1.,6.,num_obs)*np.ones((len(vlist),num_obs))
